...
void test_inserir_filas_usuario() {
    ...
    q.processo_a_ser_executado(p1);
    q.processo_a_ser_executado(p2);
    q.processo_a_ser_executado(p3);
...
void test_ordem_fifo_tempo_real() {
    ...
    q.processo_a_ser_executado(p0);
    q.processo_a_ser_executado(p1);
    q.processo_a_ser_executado(p2);

    // Deve sair na ordem de entrada (FIFO)
    ASSERT_EQ(0, q.proximo_processo_a_ser_executado().id);
    ASSERT_EQ(1, q.proximo_processo_a_ser_executado().id);
    ASSERT_EQ(2, q.proximo_processo_a_ser_executado().id);
}
...
void test_tempo_real_tem_prioridade_absoluta() {
    ...
    q.processo_a_ser_executado(user);
    q.processo_a_ser_executado(rt);

    // Tempo real deve sair primeiro independente da ordem de inserção
    BCP first = q.proximo_processo_a_ser_executado();
...
void test_ordem_de_prioridade_usuario() {
    ...
    ASSERT_EQ(10, q.proximo_processo_a_ser_executado().id);
    ASSERT_EQ(20, q.proximo_processo_a_ser_executado().id);
    ASSERT_EQ(30, q.proximo_processo_a_ser_executado().id);
}
...
void test_remocao_seta_estado_executando() {
    ...
    q.processo_a_ser_executado(p);
    BCP out = q.proximo_processo_a_ser_executado();
...
void test_filas_vazias() {
    ...
    q.proximo_processo_a_ser_executado();
    ASSERT_TRUE(q.todas_as_filas_estao_vazias());
}
...
void test_remocao_em_fila_vazia_lanca_excecao() {
    GerenciadorDeFilas q;
    bool lancou = false;
    try { q.proximo_processo_a_ser_executado(); }
...
void test_devolucao_apos_preempcao() {
    ...
    BCP p = criar_processo(0, 1);
    q.processo_a_ser_executado(p);
    BCP executando = q.proximo_processo_a_ser_executado();
